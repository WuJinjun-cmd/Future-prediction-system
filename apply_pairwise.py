...
with open(r'd:\Future-prediction-system\pairwise_scores.json', 'r', encoding='utf-8') as f:
    scores = json.load(f)
    ...
    for major_name in majors:
        ...
        if match:
            ...
            if match.start() < kw_pos:
                ...
                applied += 1
                print(f"  {major_name}: {score} (优势{info['avg_advantage']:+.0f}, {info['pairs']}组对战, {info['unis']}所)")
